
s_box = [
    
]

def print_state(state, title):
    print(title)
    for row in state:
        print(" ".join(f"{byte:02x}" for byte in row))
    print()

def sub_bytes(state):
    """
    SubBytes transformation - substitutes each byte in the state with 
    the corresponding value in the S-box
    """
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]
    return state

def shift_rows(state):
    """
    ShiftRows transformation - cyclically shifts each row to the left
    Row 0: No shift
    Row 1: Shifted 1 position to the left
    Row 2: Shifted 2 positions to the left
    Row 3: Shifted 3 positions to the left
    """
    
    state[1] = state[1][1:] + state[1][:1]

    state[2] = state[2][2:] + state[2][:2]
 
    state[3] = state[3][3:] + state[3][:3]
    return state

def demonstrate_aes_steps():

    state = [
        [0x19, 0xa0, 0x9a, 0xe9],
        [0x3d, 0xf4, 0xc6, 0xf8],
        [0xe3, 0xe2, 0x8d, 0x48],
        [0xbe, 0x2b, 0x2a, 0x08]
    ]
    
    print_state(state, "Initial State:")
   
    state = sub_bytes(state)
    print_state(state, "After SubBytes:")
    
    state = shift_rows(state)
    print_state(state, "After ShiftRows:")
    
    return state

if __name__ == "__main__":
    print("AES: Demonstrating SubBytes and ShiftRows Steps")
    print("=" * 45)
    result = demonstrate_aes_steps()
    

    expected_after_shift_rows = [
        [0xd4, 0xe0, 0xb8, 0x1e],
        [0x27, 0xbf, 0xb4, 0x41],
        [0x11, 0x98, 0x5d, 0x52],
        [0xae, 0xf1, 0xe5, 0x30]
    ]